...
from xfusion.train.basicsr.data.xray_dataset_PHSH2 import XrayDatasetSTF, XrayVideoTestDatasetSTF
...
def parse_options_(root_path,args,opt):
    ...
    if args.pretrain_network_g != 'none':
        ...
        opt['path']['resume_state'] = Path(args.pretrain_network_g).parents[1] / 'training_states' / (Path(args.pretrain_network_g).stem.split('_')[-1]+ '.state')
        
        assert os.path.isfile(opt['path']['resume_state'])
        opt['path']['strict_load_g'] = True
        opt['path']['ignore_resume_networks'] = 'network_g'
        print(f"to load model and state from {opt['path']['pretrain_network_g']} and {opt['path']['resume_state']}")
    ...
    opt['val']['val_freq'] = args.val_freq
    opt['logger']['save_checkpoint_freq'] = args.val_freq
    postfix = f"_{args.drop_path_rate}{args.iter_div_factor}{args.num_frame}{args.embed_dim}{args.batch_size}{args.initial_lr}{args.num_workers}{args.warmup_iter}{args.scale_depth}{args.scale_mha}{args.window_size_spatial}{args.dataset_enlarge_ratio}{args.sub_dataset}{args.sub_dataset_frac}{args.resi_connection}{args.conv_window_size}{args.profiler_iter_num}{args.single_epoch_ok}_renorm_base_rem1_world_size_{opt['world_size']}"
    opt['name'] += postfix
    # random seed
