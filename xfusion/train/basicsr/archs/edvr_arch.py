...
class InversePCDAlignment(nn.Module):
    """Alignment module using Pyramid, Cascading and Deformable convolution
    (PCD). It is used in EDVR.
    ...
    def __init__(self, num_feat=64, deformable_groups=8):
        super(InversePCDAlignment, self).__init__()
    ...
    def forward(self, nbr_feat_l, ref_feat_l, fused_feat_l):
        """Align neighboring frame features to the reference frame features.
        ...
        Args:
            ...
            ref_feat_l (list[Tensor]): Reference feature list. It
                contains three pyramid levels (L1, L2, L3),
                each with shape (b, c, h, w).
            fused_feat_l (list[Tensor]): Post fusion neighboring feature list. It
                contains three pyramid levels (L1, L2, L3),
                each with shape (b, c, h, w).
        ...
        for i in range(3, 0, -1):
            ...
            feat = self.dcn_pack[level](fused_feat_l[i - 1], offset)
            if i < 3:
                feat = self.feat_conv[level](torch.cat([feat, upsampled_feat], dim=1))
            if i > 1:
                feat = self.lrelu(feat)
