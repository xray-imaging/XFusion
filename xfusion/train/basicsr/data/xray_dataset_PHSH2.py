...
# Modifications Copyright 2025 xfusion authors
...
class XrayDatasetSTF(data.Dataset):
    def __init__(self, opt):
        ...
        if opt['name'].lower() in ['redsctc','xray']:
            ...
            if 'dataroot_context' in list(opt.keys()):
                self.context_root = [Path(cr) for cr in opt['dataroot_context']]
        else:
            if type(opt['dataroot_gt']) is str:
                self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(opt['dataroot_lq'])
                if 'dataroot_context' in list(opt.keys()):
                    self.context_root = Path(opt['dataroot_context'])
            else:
        ...
        self.case_gt_root = {}
        self.case_lq_root = {}
        self.case_context_root = {}
        self.context_frame_num = {}
        self.mask_lr = opt.get('mask_lr', False)
        self.context_map_file = opt.get('meta_info_context', None)
        if self.context_map_file is not None:
            if type(self.context_map_file) is list:
                for i, cmf in enumerate(self.context_map_file):
                    if i == 0:
                        self.context_map = pd.read_csv(cmf,index_col=None)
                    else:
                        context_map = pd.read_csv(cmf,index_col=None)
                        self.context_map = pd.concat((self.context_map,context_map),ignore_index=True)

            elif type(self.context_map_file) is str:
                self.context_map = pd.read_csv(self.context_map_file,index_col=None)
        self.use_nearest_context = opt.get('use_nearest_context', False)
        ...
        if opt['name'].lower() in ['redsctc','xray']:
            ...
            with open(opt['meta_info_file'], 'r') as fin:
                for line in fin:
                    ...
                    self.case_context_root[folder] = self.context_root
                    self.context_frame_num[folder] = int(context_frame_num)
                    self.keys.extend([f'{folder}/{i:08d}' for i in range(int(frame_num))])
    ...
    def __getitem__(self, index):
        ...
        # keep the rejection sampling within the paired frame indices as the frame separation is symmetric anyway
        ...
        if self.use_nearest_context:
            interval_ = random.choice(list(range(1,interval * self.context_map[(self.context_map['case']==int(clip_name)) & (self.context_map['paired index']==0)].iloc[0]['unpaired index']+1)))
            center_frame_idx_ = self.context_map[(self.context_map['case']==int(clip_name)) & (self.context_map['paired index']==center_frame_idx)].iloc[0]['unpaired index']
            start_frame_idx_ = center_frame_idx_ - self.num_half_frames * interval_
            end_frame_idx_ = center_frame_idx_ + self.num_half_frames * interval_
            assert start_frame_idx_ >= 0
            assert end_frame_idx_ <= (self.context_frame_num[clip_name] - 1)
            neighbor_list = list(range(start_frame_idx_, end_frame_idx_+1, interval_))
        else:
        ...
        if not self.multi_recon_ok:
            ...
            # disable multi_recon as GT frames are not densely available as the LR
            if not self.use_nearest_context:
                img_gt = []
                ...
                raise Exception("multiple reconstruction for nearest LR images not implemented")
        ...
        #TODO: w and w.o. use_nearest_context enabled
        img_lqs = []
        for neighbor in neighbor_list:
            if self.use_nearest_context:
                img_lq_path = self.case_context_root[clip_name] / clip_name / f'{neighbor:08d}.png'
            else:
        ...
        # randomly crop
        if self.opt['num_frame_hi'] == 0:
            if (img_gt.shape[0] == img_lqs[0].shape[0]) and (img_gt.shape[1] == img_lqs[0].shape[1]):
                img_gt, img_lqs = paired_random_crop(img_gt, img_lqs, gt_size, 1, img_gt_path, img_hqs = img_hqs, patch_corner = patch_corner)
                img_lqs = [cv2.resize(img_lq, (img_lqs[0].shape[1]//scale,img_lqs[0].shape[0]//scale), interpolation=cv2.INTER_LINEAR)[...,None] for img_lq in img_lqs]
            else:
                img_gt, img_lqs = paired_random_crop(img_gt, img_lqs, gt_size, scale, img_gt_path, img_hqs = img_hqs, patch_corner = patch_corner)
        else:
            if (img_gt.shape[0] == img_lqs[0].shape[0]) and (img_gt.shape[1] == img_lqs[0].shape[1]):
                img_gt, img_lqs, img_hqs = paired_random_crop(img_gt, img_lqs, gt_size, 1, img_gt_path, img_hqs = img_hqs, patch_corner = patch_corner)
                img_lqs = [cv2.resize(img_lq, (img_lqs[0].shape[1]//scale,img_lqs[0].shape[0]//scale), interpolation=cv2.INTER_LINEAR)[...,None] for img_lq in img_lqs]
            else:
        ...
        if not self.mask_lr:
            ...
            img_lqs_ = img_results[0:-(num_gts+self.opt['num_frame_hi'])]
            lq_mask = (img_lqs_[0] == 0).detach().clone()
            img_lqs_ = [lq / img_lqs_[0] for lq in img_lqs_]
            img_lqs_ = [lq.masked_fill(lq_mask,0) for lq in img_lqs_]
            img_lqs = torch.stack(img_lqs_, dim=0)
        ...
        if self.flow_root is not None:
            if img_hqs is not None:
                sample = {'lq': img_lqs, 'flow': img_flows, 'gt': img_gt, 'key': key, 'hq': img_hqs}
            else:
                sample = {'lq': img_lqs, 'flow': img_flows, 'gt': img_gt, 'key': key}
        else:
            if img_hqs is not None:
                sample = {'lq': img_lqs, 'gt': img_gt, 'key': key, 'hq': img_hqs}
            else:
                sample = {'lq': img_lqs, 'gt': img_gt, 'key': key}
        if self.mask_lr:
            sample['mask_lr'] = True
        return sample
...
class XrayVideoTestDatasetSTF(data.Dataset):
    def __init__(self, opt):
        ...
        if opt['name'].lower() in ['vid4', 'reds4', 'redsofficial','redsctc','xray']:
    ...
    def __getitem__(self, index):
        ...
        if self.mask_lr:
            imgs_div = imgs_lq[0,...].unsqueeze(0).repeat(imgs_lq.size()[0],1,1,1).detach().clone()
            imgs_lq = imgs_lq / imgs_div
            imgs_lq.masked_fill_(imgs_div == 0, 0)
